...
class TestFindAllC4BFSDIMACS(unittest.TestCase):
    """
    Testing find_all_c4_bfs_broken(nx.Graph) function on 25-node subgraphs of DIMACS graphs.
    Asserts for each case include:
        Confirming all C4s found are valid C4s.
        Confirming all nodes in the C4s are valid in the graph.
        Confirming there are no duplicate C4s found.
        Confirming that the C4 list is non-negative.
    """
    # Custom Assertion
    def assertIsC4s(self, G, c4_list):
        for c4 in c4_list:
            self.assertTrue(is_c4(G, list(c4)))
...
# DIMACS graphs - 25-vertex subgraph
    def test_brock200_2(self):
        ...
        c4_list = find_all_c4_bfs_broken(H)

        self.assertIsC4s(G, c4_list)
