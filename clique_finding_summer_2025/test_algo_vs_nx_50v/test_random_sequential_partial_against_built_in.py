...
from functions.clique_algorithms import custom_with_partial_greedy
from functions.read_DIMACS import read_dimacs_clq

# 37 Tests and 222 Asserts
class TestRandomSequentialAgainstBuiltIn(unittest.TestCase):
    """
    Compares outputs of custom_with_greedy(nx.Graph, mode) to networkX built-in function find_cliques(nx.Graph)
    ---A VARIANT OF BRON KERBOSCH PRUNING USING RANDOM SEQUENTIAL GREEDY COLOURING HEURISTIC---
    ---PARTIAL GREEDY REFERS TO THE COLOURING HEURISTIC BEING USED EVERY 3 ITERATIONS---
    Each test compares output using a 50-node subgraph of all DIMACS graphs.
    Asserts for each case run through:
        ...
        A sanity test confirming the clique size does not exceed 50
    ...
    # DIMACS graphs - 50-vertex subgraph
    def test_brock200_2(self):
        ...
        max_c = custom_with_partial_greedy(G, "random_sequential")
        ...
        self.assertLessEqual(len(max_c), 50) # Clique size <= 50 (size of H)
