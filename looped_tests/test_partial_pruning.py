...
def make_test(graph, index, cat):
    ...
    def test(self):
        try:
            ...
            t_2 = dt.now()
            c_max_clq, c_steps = max_clique_with_steps(graph)
            c_runtime = dt.now() - t_2

            # Custom with largest_first colouring function
            t_3 = dt.now()
            gl_max_clq, gl_steps = custom_with_partial_greedy_steps(graph, "largest_first")
            gl_runtime = dt.now() - t_3

            # Custom with random_sequential colouring function
            t_4 = dt.now()
            gr_max_clq, gr_steps = custom_with_partial_greedy_steps(graph, "random_sequential")
